...
const CardNav: React.FC<CardNavProps> = ({
    ...
    return (
        <div
            className={`card-nav-container sticky top-0 w-full z-[100] shadow-[0_8px_32px_0_rgba(31,38,135,0.15),inset_0_2px_3px_rgba(255,255,255,0.9),inset_0_-1px_3px_rgba(0,0,0,0.05)] border-b border-white/60 backdrop-blur-[24px] ${className}`}
            style={{
        ...
        >
            <nav
                ref={navRef}
                className={`card-nav mx-auto w-[96%] max-w-[1800px] ${isExpanded ? 'open' : ''} block h-[60px] p-0 relative overflow-hidden will-change-[height]`}
                style={{ backgroundColor: baseColor === 'transparent' ? 'transparent' : baseColor }}
            >
                ...
                >
                    {(items || []).slice(0, 3).map((item, idx) => (
                        <div
                            key={`${item.label}-${idx}`}
                            className="nav-card select-none relative flex flex-col gap-2 p-[12px_16px] rounded-xl min-w-0 flex-[1_1_auto] h-auto min-h-[60px] md:h-full md:min-h-0 md:flex-[1_1_0%] border border-white/60 shadow-[0_8px_16px_rgba(0,0,0,0.05)] backdrop-blur-md transition-all hover:bg-white/50"
                            ref={setCardRef(idx)}
